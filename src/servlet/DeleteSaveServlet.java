...
/**
 * Servlet implementation class DeleteSaveServlet
 */
@WebServlet("/deleteSaveServlet")
public class DeleteSaveServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean result1 = mss.deleteSave(userid, postid);
		if(!result1){
			response.getWriter().write("failed");
		}else{
			boolean result2 = mps.deleteSaveNum(postid);
			if(!result2){
				response.getWriter().write("failed");
			}else{
